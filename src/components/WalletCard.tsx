export function WalletCard({
    ...
    return (
        <div className="border border-white/10 hover:border-white/20 transition-colors duration-200">
            {/* Header */}
            <div className="flex items-center justify-between px-5 py-4 border-b border-white/[0.07] bg-white/[0.015]">
                <div className="flex items-center gap-3">
                    ...
                    <span className="text-base font-semibold text-white">Wallet {index + 1}</span>
                </div>
                <div className="flex items-center gap-3">
                    <span className="text-xs text-zinc-700 font-mono">index / {index}</span>
                    <button
                        onClick={onRemove}
...
function KeyField({
    ...
    return (
        <div className="flex items-center gap-4 px-5 py-4">
            <span className="text-xs text-zinc-600 font-mono uppercase tracking-wider w-20 shrink-0">{label}</span>
            <span className={`flex-1 font-mono text-sm truncate transition-all ${revealed ? "text-zinc-200" : "text-zinc-700 tracking-[0.15em]"}`}>
                {display}
            </span>
            <div className="flex items-center gap-2 shrink-0">
                {sensitive && onToggle && (
                    <button
                        onClick={onToggle}
                        className="p-1.5 text-zinc-700 hover:text-zinc-300 transition-colors border border-transparent hover:border-white/10"
...
export function EmptyState({ color, message }: { color: string; message: string }) {
    return (
        <div className="border border-dashed border-white/[0.08] py-14 flex flex-col items-center justify-center gap-3">
            <div
